...
export async function getAllBlogPosts(): Promise<BlogPost[]> {
  ...
  try {
    ...
    for (const blog of dbBlogs) {
      const processedContent = await remark().use(html, { sanitize: false, allowDangerousHtml: true }).process(blog.content);
...
export async function getBlogPost(
  ...
  try {
    ...
    if (dbBlog) {
      const processedContent = await remark().use(html, { sanitize: false, allowDangerousHtml: true }).process(dbBlog.content);
      return {
        frontmatter: {
          title: dbBlog.title,
          slug: dbBlog.slug,
